...
如果本地不存在镜像，Docker Compose 会自动构建镜像。容器启动后，`entrypoint.sh` 会自动加载 ROS Noetic 环境，编译 `/workspace/catkin_ws`，加载 `/workspace/catkin_ws/devel/setup.bash`，启动 noVNC 图形桌面，并运行轨迹跟踪实验。
...
进入桌面后可以看到 RViz，机器人会在仿真环境中跟随目标轨迹运动。
